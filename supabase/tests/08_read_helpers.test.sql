...
-- 5. public.vehicles is in the supabase_realtime publication.
select is(
  (select count(*) from pg_publication_tables where pubname='supabase_realtime' and tablename='vehicles'),
  1::bigint, 'vehicles in realtime publication');
