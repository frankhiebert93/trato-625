        ...
        <div onClick={onClick} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col w-full cursor-pointer active:scale-[0.98] transition-transform relative">
            <div className="w-full h-56 bg-gray-200 relative overflow-hidden">
                ...
                {photoCount > 1 && !item.is_sold && (
                    <span className="absolute top-3 left-3 bg-black/60 backdrop-blur-sm text-white text-[11px] font-bold px-2.5 py-1 rounded-md shadow-sm flex items-center gap-1 z-20">
                ...
                {item.category && !item.is_sold && (
                    <span className="absolute top-3 right-3 bg-slate-900/80 backdrop-blur-sm text-white text-[11px] font-bold px-3 py-1.5 rounded-full shadow-sm z-20">
                        {item.category}
                    </span>
                )}
            </div>
